...
const CumulativeGPACalc = (props) => {
  const [cumulativeGPA, setCumulativeGPA] = useState("");
  const [totalCumulativeCredits, setTotalCumulativeCredits] = useState("");
  const [courses, setCourses] = useState(
    Array(props.numRows).fill({ courseName: "", level: "", grade: "", credits: "", gradePoints: 0, qualityPoints: 0 })
  );
  ...
  const calculateGPA = (courses) => {
    ...
    const cumulativeQualityPoints = cumulativeGPA * totalCumulativeCredits;
    const currentQualityPoints = totalQualityPoints;

    const totalQualityPointsAll = cumulativeQualityPoints + currentQualityPoints;
    const totalCreditsAll = totalCumulativeCredits + totalCredits;

    setGpa(totalCreditsAll ? totalQualityPointsAll / totalCreditsAll : 0);
  };
  ...
  return (
    <div className="flex flex-col self-start justify-self-center sm:p-2">
      ...
      {/* Inputs for cumulative GPA and total credits */}
      <div className="mb-4">
        <label>
          Cumulative GPA:
          <input
            type="number"
            value={cumulativeGPA}
            onChange={(e) => setCumulativeGPA(parseFloat(e.target.value))}
        ...
        <label className="ml-4">
          ...
          <input
            type="number"
            value={totalCumulativeCredits}
            onChange={(e) => setTotalCumulativeCredits(parseFloat(e.target.value))}
            className="ml-2 p-1 border"
          />
        </label>
      </div>
...
export default CumulativeGPACalc;
